...
class Encoder(nn.Module):
    ...
    def forward(self, commands, args, label=None):
        ...
        if self.cfg.model_type == "transformer":
            ...
            hidden_cell = (src.new_zeros(2, N, self.cfg.d_model // 2), src.new_zeros(2, N, self.cfg.d_model // 2))
    ...
    def __init__(self, cfg: _DefaultConfig):
        ...
        if cfg.pred_mode == "autoregressive":
            self.embedding = SVGEmbedding(
                cfg, cfg.max_total_len, rel_args=cfg.rel_targets, use_group=True, group_len=cfg.max_total_len
            )
...
class SVGTransformer(nn.Module):
    ...
    def perfect_matching(self, command_logits, args_logits, hierarch_logits, tgt_commands, tgt_args):
        with torch.no_grad():
            ...
            tgt_args, args_logits = tgt_args.repeat(1, 1, self.cfg.num_groups_proposal, 1, 1), args_logits.repeat(
                1, G, 1, 1, 1, 1
            )
            ...
            loss_args = F.cross_entropy(
                args_logits.reshape(-1, self.args_dim), tgt_args.reshape(-1).long() + 1, reduction="none"
            ).reshape(
                N, G, self.cfg.num_groups_proposal, S, n_args
            )  # shift due to -1 PAD_VAL
            loss_cmd = F.cross_entropy(
                command_logits.reshape(-1, self.cfg.n_commands), tgt_commands.reshape(-1).long(), reduction="none"
    ...
    def forward(
        ...
        out_logits = self.decoder(
            z, commands_dec_, args_dec_, label, hierarch_logits=hierarch_logits, return_hierarch=return_hierarch
        )
        ...
        if return_tgt and self.cfg.self_match:  # Assignment
            ...
            assignment = self.perfect_matching(
                command_logits, args_logits, hierarch_logits, commands_dec[..., 1:], args_dec[..., 1:, :]
            )
    ...
    def greedy_sample(
        ...
        if self.cfg.pred_mode == "one_shot":
            ...
            for i in range(self.cfg.max_total_len):
                res = self.forward(
                    None, None, commands_y, args_y, label=label, z=z, hierarch_logits=hierarch_logits, return_tgt=False
                )
        ...
        if concat_groups:
            ...
            commands_y, args_y = commands_y[padding_mask_y].reshape(N, -1), args_y[padding_mask_y].reshape(
                N, -1, self.cfg.n_args
            )
