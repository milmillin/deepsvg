...
class SVGCommand:
    ...
    def from_tensor(vector: torch.Tensor):
        ...
        return SVGCommand.from_args(
            cmd, radius, x_axis_rotation, large_arc_flag, sweep_flag, start_pos, control1, control2, end_pos
        )
...
class SVGCommandLinear(SVGCommand):
    ...
    def to_tensor(self, PAD_VAL=-1):
        ...
        return torch.tensor(
            [cmd_index, *([PAD_VAL] * 5), *self.start_pos.to_tensor(), *([PAD_VAL] * 4), *self.end_pos.to_tensor()]
        )
...
class SVGCommandArc(SVGCommand):
    def __init__(
        ...
        super().__init__(
            SVGCmdEnum.ELLIPTIC_ARC, [radius, x_axis_rotation, large_arc_flag, sweep_flag, end_pos], start_pos, end_pos
        )
    ...
    def reverse(self):
        return SVGCommandArc(
            self.end_pos, self.radius, self.x_axis_rotation, self.large_arc_flag, ~self.sweep_flag, self.start_pos
        )
